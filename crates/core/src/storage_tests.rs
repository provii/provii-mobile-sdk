...
// Test-only support and tests relocated verbatim from storage.rs.
// This file is the body of `mod storage_tests`, declared via #[path] in
// storage.rs. `use super::*;` re-imports the storage module's items so the
// nested `mod storage_data` and `mod tests` resolve `super::*` to storage,
// exactly as they did when inlined.
use super::*;
